...
module.exports = (model) => {
  ...
  for (const association of model.getModelAssociations()) {
    ...
    if (association.associationType === 'HasOne' || association.associationType === 'BelongsTo') {
      ...
      result.additionalProperties[attribute] = reference;
    } else {
      const attribute = typeof association.as === 'string' ? association.as : association.as.plural;
      result.additionalProperties[attribute] = {type: 'array', items: reference};
    }
  }
  return result;
};

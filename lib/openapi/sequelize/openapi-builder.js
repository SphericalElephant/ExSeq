...
class SequelizeOpenApiBuilder extends OpenApiBuilder {
  ...
  static createModelSchemasRecursive(model, existingSchemas, getModelOpts) {
    let schemas = {...existingSchemas};
    if (!schemas[model.name]) {
      const schema = SequelizeOpenApiBuilder.convert(model);

      const createExcludeFields = new Set(getModelOpts(model).createExcludeFields || ['id', 'createdAt', 'updatedAt']);
    ...
    for (const associationName in model.associations) {
      ...
      if (!schemas[targetModel.name]) {
        schemas = SequelizeOpenApiBuilder.createModelSchemasRecursive(targetModel, schemas, getModelOpts);
      }
    }
    return schemas;
  }
  ...
  createModelRequestBody(model, operation) {
    const modelRef = {$ref: `#/components/schemas/${model.name}`};
    if (operation === 'post') {
      const postSchemaName = SequelizeOpenApiBuilder.createCreateModelSchemaName(model);
      if (this.existingSchemaNames.includes(postSchemaName)) {
        modelRef.$ref = `#/components/schemas/${postSchemaName}`;
      }
    ...
    const requestBodyName = `${model.name}`;
    return {
  ...
  createModelInstanceResponse(model, operation) {
    const schemaName = `${model.name}Response`;
    const responseName = `${model.name}`;
    const modelRef = {$ref: `#/components/schemas/${model.name}`};
    if (operation === 'get') {
      const optionalSchemaName = SequelizeOpenApiBuilder.createOptionalModelSchemaName(model);
      if (this.existingSchemaNames.includes(optionalSchemaName)) {
  ...
  createModelArrayResponse(model) {
    const schemaName = `${model.name}ArrayResponse`;
    const responseName = `${model.name}`;
    const modelRef = {$ref: `#/components/schemas/${model.name}`};
    const optionalSchemaName = SequelizeOpenApiBuilder.createOptionalModelSchemaName(model);
    if (this.existingSchemaNames.includes(optionalSchemaName)) {
  ...
  createModelSearchResponse(model, empty) {
    const schemaName = `${model.name}SearchResponse`;
    const responseName = empty ? `${model.name}SearchEmpty` : `${model.name}Search`;

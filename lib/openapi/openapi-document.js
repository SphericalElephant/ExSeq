...
class OpenApiDocument {
  ...
  addComponents(components) {
    for (const componentType in components) {
      const addComponent = `add${componentType.charAt(0).toUpperCase()}${componentType.slice(1)}`;
      if (typeof this[addComponent] !== 'function') {
        console.warn('Component type not supported by OpenApiDocument.addComponents:', componentType);
      } else {
        this[addComponent](components[componentType]);
      }
    }
  }

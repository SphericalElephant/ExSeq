...
class QueryBuilder {
  constructor(
    {models = [], model = {}, defaultLimit = 10, maxLimit = NONE, whitelistedOperators = undefined} = {}
  ) {
  ...
  /**
   * Constructs a new instance of QueryBuilder from a given instance. Will not replicate the internal state of the
   * QueryBuilder.
   * @param {QueryBuilder} otherQueryBuilder The QueryBuilder that should be used to construct the new instance
   */
  static from(otherQueryBuilder) {
    const newInstance = new QueryBuilder();
    newInstance.defaultLimit = otherQueryBuilder.defaultLimit;
    newInstance.maxLimit = otherQueryBuilder.maxLimit;
    newInstance.whitelistedOperators = otherQueryBuilder.whitelistedOperators;
    newInstance.models = otherQueryBuilder.models;
    newInstance.model = otherQueryBuilder.model;
    newInstance.queryField = null;
    newInstance.prepared = false;
    return newInstance;
  }
  ...
  _createOrderArrayFromInput(order) {
    if (!order) return order;
    // only works for a single query item
    const parseResult = this._parseOrderFieldString(order[0][0]);
    if (typeof parseResult === 'string')
      return [[parseResult, order[0][1]]];
    else if (Array.isArray(parseResult)) {
      return [[this._addAliasToAssociatedModel(parseResult[0]), parseResult[1], order[0][1]]];
    }
  }

  _addAliasToAssociatedModel(associatedModel) {
    for (const association in this.model.associations) {
      if (this.model.associations[association].target.name === associatedModel.name) {
        return {model: associatedModel, as: association};
      }
    }
  }
  ...
  prepare() {
    ...
    this.queryField.order = this._createOrderArrayFromInput(this.queryField.order);
    this.prepared = true;

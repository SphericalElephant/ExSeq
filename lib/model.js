...
module.exports = (model) => {
  model.exseqGetUpdateableAttributes = function () {
  ...
  model.removeIllegalAttributes = function (input) {
    return _.pick(input, this.exseqGetUpdateableAttributes().map(attr => attr.attribute));
  };

  model.fillMissingUpdateableAttributes = function (association, source, input) {
    const result = this.exseqGetUpdateableAttributes().reduce((result, current) => {
